...
object BayouSynthesizer {
    ...
    operator fun invoke(type: SynthesizerType, request: BayouRequest, progress: SynthesisProgress): BayouResponse? {
        val model = models[type] ?: BayouClient.getConfigurableModel(InputStreamReader(BayouSynthesizer::class.java.classLoader.getResourceAsStream("${type.name.toLowerCase()}.json")).readText())
